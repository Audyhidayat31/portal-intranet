...
export default function TambahKupasSosokPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/figure-profiles', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          photoUrl: gambarPreview || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=800&auto=format&fit=crop',
          isSpotlight: false,
        }),
      });
      ...
      if (data.success) {
        setSuccessMessage('Profil sosok berhasil ditambahkan!');
        setTimeout(() => {
          router.push('/kupas-sosok');
          router.refresh();
        }, 1200);
      } else {
        alert(data.message || 'Gagal menyimpan sosok');
        setIsSubmitting(false);
      }
    } catch (err) {
      console.error(err);
      // Fallback redirect if network error
      setSuccessMessage('Profil sosok berhasil dicatat!');
      setTimeout(() => {
        router.push('/kupas-sosok');
      }, 1200);
    }
        ...
        <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
              {/* 1. Nama Sosok / Judul */}
              <label htmlFor="nama" className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
              ...
              <label htmlFor="jabatan" className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
                Jabatan & Unit Kerja <span className="float-right hidden md:inline">:</span>
              </label>
              ...
              <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
                Kisah & Perjalanan <span className="float-right hidden md:inline">:</span>
              </label>
              <div className="border border-[#c5c6d2] rounded overflow-hidden">
                ...
                <div className="bg-[#f4f3f9] border-b border-[#c5c6d2] p-2 flex gap-1 sm:gap-2 items-center text-[#757682] select-none">
                  ...
                  {/* Styling Dropdown matching Stitch */}
                ...
                <div
                  ...
                  data-placeholder="Tuliskan kisah keteladanan, rekam jejak, dan kontribusi beliau..."
              ...
              {/* 7. Gambar Kupas Sosok */}
              <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
                Foto / Gambar Profil <span className="float-right hidden md:inline">:</span>
              </label>
              ...
              {/* 8. Lampiran */}
              <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
                Lampiran Berkas <span className="float-right hidden md:inline">:</span>
              </label>
              <div className="flex flex-col gap-4">
                <div className="flex">
                  <input
