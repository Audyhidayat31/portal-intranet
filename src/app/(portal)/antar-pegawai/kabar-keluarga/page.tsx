...
import {
  STITCH_MOCK_KABAR_KELUARGA_9,
  KabarKeluargaItem,
} from '@/lib/mock-kabar-keluarga';
...
export default function KabarKeluargaPage() {
  ...
  const fetchKabarKeluarga = (q: string = '') => {
    if (q.trim()) {
      setIsLoading(true);
    }
    fetch(`/api/employee-posts?category=kabar-keluarga&q=${encodeURIComponent(q)}`)
      .then((res) => res.json())
      .then((data) => {
        const apiItems = data.success && Array.isArray(data.data) ? data.data : [];

        const formattedApiItems: KabarKeluargaItem[] = apiItems.map((item: any, idx: number) => ({
          id: item.id,
          title: item.title,
        ...
        // Keep the 9 Stitch cards on page 1 matching Google Stitch design, followed by DB posts
        const pool = [...STITCH_MOCK_KABAR_KELUARGA_9];
      ...
      .catch(() => {
        let filtered = STITCH_MOCK_KABAR_KELUARGA_9;
        if (q.trim()) {
          filtered = filtered.filter(
            (item) =>
              item.title.toLowerCase().includes(q.toLowerCase()) ||
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    fetchKabarKeluarga(searchQuery);
  };
    ...
    <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-8 md:py-10 bg-[#faf8ff] text-[#1a1b20] min-h-[calc(100vh-80px)] flex flex-col justify-between">
      <div>
        ...
        <nav aria-label="Breadcrumb" className="mb-8 text-sm text-[#444650] flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4 text-[#757682]" />
          <span className="text-[#00113a] font-semibold">Kabar Keluarga</span>
        </nav>

        {/* Header Section matching Google Stitch */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <Link
            href="/antar-pegawai/kabar-keluarga/tambah"
        ...
        {/* Search Bar matching Google Stitch */}
          ...
          <form onSubmit={handleSearchSubmit} className="flex gap-2 w-full">
            <div className="relative flex-grow">
              <input
                ...
                placeholder="Cari Kabar Keluarga..."
        ...
        {/* Grid of Cards: Exactly 9 Cards in 3 Columns matching Google Stitch */}
        {isLoading ? (
          ...
          <div className="text-center py-16 border border-dashed border-[#c5c6d2] rounded-xl bg-white my-8">
            ...
            <h2 className="text-lg font-bold text-[#00113a] mb-1">Tidak Ada Kabar Keluarga Ditemukan</h2>
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-grow justify-between">
                    ...
                    <div className="flex justify-end pt-2 mt-auto">
                      <Link
                        href={`/antar-pegawai/kabar-keluarga/${item.id}`}
