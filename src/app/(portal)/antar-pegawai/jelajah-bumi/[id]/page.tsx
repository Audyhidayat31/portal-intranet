...
export default function DetailJelajahBumiPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setEditTitle(DEFAULT_STITCH_JELAJAH_DETAIL.title);
      setEditContent(DEFAULT_STITCH_JELAJAH_DETAIL.content);
      setIsLoading(false);
      return;
    }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedbackMsg('Catatan Jelajah Bumi berhasil diperbarui!');
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-[#c5c6d2] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdate} className="space-y-4 mt-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[#1a1b20] mb-1">Uraian Jelajah Bumi</label>
                <textarea
                  rows={8}
                  required
                  value={editContent}
