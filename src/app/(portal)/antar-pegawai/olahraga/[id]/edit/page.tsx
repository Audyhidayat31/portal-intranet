...
export default function EditOlahragaPage() {
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          if (item.coverImage) {
            ...
            setGambarFileName('sampul-olahraga.jpg');
          }
        } else if (matchedMock) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judul.trim()) {
      alert('Nama judul Kegiatan Olahraga wajib diisi');
      return;
    }
    ...
    try {
      ...
      const stored = getStoredOlahraga();
      const updated = stored.map((item) =>
        item.id === rawId
          ? {
              ...
              body: htmlContent || rawText || judul,
              coverImage: gambarPreview || item.coverImage,
            }
          : item
      );
      saveStoredOlahraga(updated);

      setSuccessMessage('Perubahan Kegiatan Olahraga berhasil disimpan!');
      setTimeout(() => {
        router.push(`/antar-pegawai/olahraga/${rawId}`);
      }, 1000);
    } catch {
      setSuccessMessage('Perubahan Kegiatan Olahraga berhasil disimpan (mode lokal)!');
      setTimeout(() => {
        router.push(`/antar-pegawai/olahraga/${rawId}`);
      }, 1000);
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-16 flex flex-col items-center justify-center text-[#757682]">
        ...
        <p className="text-sm font-medium">Memuat data Kegiatan Olahraga...</p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
            ...
            <div>
              <input
                ...
                placeholder="Nama judul Kegiatan Olahraga"
            ...
            <div className="border border-[#c5c6d2] rounded overflow-hidden">
              ...
              <div
                ...
                data-placeholder="Tulis deskripsi kegiatan olahraga..."
          ...
          <div className="flex gap-4 pt-6 mt-8 border-t border-[#c5c6d2]">
            ...
            <Link
              href={`/antar-pegawai/olahraga/${rawId}`}
