...
export default function EditHumorPage() {
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          if (item.coverImage) {
            ...
            setGambarFileName('sampul-humor.jpg');
          }
        } else if (matchedMock) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judul.trim()) {
      alert('Nama judul Humor Pegawai wajib diisi');
      return;
    }
    ...
    try {
      ...
      setSuccessMessage('Perubahan Humor Pegawai berhasil disimpan!');
      setTimeout(() => {
        router.push(`/antar-pegawai/humor/${rawId}`);
      }, 1000);
    } catch {
      setSuccessMessage('Perubahan Humor Pegawai berhasil disimpan (mode lokal)!');
      setTimeout(() => {
        router.push(`/antar-pegawai/humor/${rawId}`);
      }, 1000);
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-16 flex flex-col items-center justify-center text-[#757682]">
        ...
        <p className="text-sm font-medium">Memuat data Humor Pegawai...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-8 md:py-10 text-[#1a1b20]">
      ...
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[#444650] text-sm mb-8 font-normal flex-wrap">
        ...
        <ChevronRight className="w-4 h-4 text-[#757682]" />
        <span className="text-[#1a1b20] font-medium">Edit Humor Pegawai</span>
      </nav>
      ...
      <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
            ...
            <div>
              <input
                ...
                placeholder="Nama judul Humor Pegawai"
            ...
            {/* 5. Gambar Humor Pegawai */}
            <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
              Gambar Humor Pegawai <span className="float-right hidden md:inline">:</span>
            </label>
          ...
          <div className="flex gap-4 pt-6 mt-8 border-t border-[#c5c6d2]">
            ...
            <Link
              href={`/antar-pegawai/humor/${rawId}`}
