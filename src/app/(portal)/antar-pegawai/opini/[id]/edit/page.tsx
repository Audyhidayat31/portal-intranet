...
export default function EditCoretanOpiniPage() {
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          if (item.coverImage) {
            ...
            setGambarFileName('sampul-opini.jpg');
          }
        } else if (matchedMock) {
  ...
  // Synchronize editor innerHTML if loaded late
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judul.trim()) {
      alert('Nama judul Coretan Opini wajib diisi');
      return;
    }
    ...
    try {
      ...
      setSuccessMessage('Perubahan Coretan Opini berhasil disimpan!');
      setTimeout(() => {
        router.push(`/antar-pegawai/opini/${rawId}`);
      }, 1000);
    } catch {
      setSuccessMessage('Perubahan Coretan Opini berhasil disimpan (mode lokal)!');
      setTimeout(() => {
        router.push(`/antar-pegawai/opini/${rawId}`);
      }, 1000);
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-16 flex flex-col items-center justify-center text-[#757682]">
        ...
        <p className="text-sm font-medium">Memuat data Coretan Opini...</p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
            ...
            <div>
              <input
                ...
                placeholder="Nama judul Coretan Opini"
            ...
            {/* 5. Gambar Coretan Opini */}
            <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
              Gambar Coretan Opini <span className="float-right hidden md:inline">:</span>
            </label>
          ...
          <div className="flex gap-4 pt-6 mt-8 border-t border-[#c5c6d2]">
            ...
            <Link
              href={`/antar-pegawai/opini/${rawId}`}
