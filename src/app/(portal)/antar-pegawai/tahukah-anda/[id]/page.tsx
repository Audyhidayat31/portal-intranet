...
export default function DetailTahukahAndaPage() {
  const params = useParams();
  const router = useRouter();
  const rawId = (params?.id as string) || '';

  const storedList = getStoredTahukahAnda();
  const matchedInitial =
    storedList.find((m) => m.id === rawId || m.title.toLowerCase().includes(rawId.toLowerCase())) ||
    STITCH_MOCK_TAHUKAH_ANDA_9.find((m) => m.id === rawId || m.title.toLowerCase().includes(rawId.toLowerCase()));

  const [tahukahItem, setTahukahItem] = useState<any>(
    matchedInitial ? { ...matchedInitial, body: matchedInitial.content } : DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL
  );
  ...
  // Edit fields
  const [editTitle, setEditTitle] = useState(matchedInitial?.title || DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL.title);
  const [editContent, setEditContent] = useState(matchedInitial?.content || DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL.content);
  ...
  useEffect(() => {
    ...
    if (foundStored) {
      setTahukahItem({ ...foundStored, body: foundStored.content });
      setEditTitle(foundStored.title);
      setEditContent(foundStored.content);
      setIsLoading(false);
      return;
    }
    ...
    fetch(`/api/employee-posts/${rawId}`)
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          const matchedMock = STITCH_MOCK_TAHUKAH_ANDA_9.find(
          ...
          if (matchedMock) {
            ...
            setTahukahItem(fullItem);
            setEditTitle(matchedMock.title);
            setEditContent(matchedMock.content);
          } else {
            setTahukahItem(DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL);
            setEditTitle(DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL.title);
            setEditContent(DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL.content);
          }
        }
        setIsLoading(false);
      })
      .catch(() => {
        const matchedMock = STITCH_MOCK_TAHUKAH_ANDA_9.find((m) => m.id === rawId);
        if (matchedMock) {
          setTahukahItem(matchedMock);
          setEditTitle(matchedMock.title);
          setEditContent(matchedMock.content);
        } else {
          setTahukahItem(DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await fetch(`/api/employee-posts/${tahukahItem?.id || rawId}`, {
        method: 'PUT',
      ...
      const updated = list.map((it) =>
        it.id === (tahukahItem?.id || rawId)
          ? { ...it, title: editTitle, content: editContent, excerpt: editContent.slice(0, 140) + '...' }
          : it
      );
      saveStoredTahukahAnda(updated);

      setTahukahItem((prev: any) => ({
        ...prev,
        title: editTitle,
        body: editContent,
        content: editContent,
      ...
      setFeedbackMsg('Artikel Tahukah Anda berhasil diperbarui!');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const list = getStoredTahukahAnda();
      const filtered = list.filter((it) => it.id !== (tahukahItem?.id || rawId));
      saveStoredTahukahAnda(filtered);
      setIsDeleting(false);
      router.push('/antar-pegawai/tahukah-anda');
    }
  };

  const displayTitle =
    tahukahItem?.title ||
    DEFAULT_STITCH_TAHUKAH_ANDA_DETAIL.title;

  const displayDate = tahukahItem?.publishedAt
    ...
    <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-8 md:py-12 bg-white min-h-[calc(100vh-80px)] flex flex-col justify-between">
      <div>
        ...
        <div className="bg-white rounded-lg p-6 sm:p-10 border border-[#c5c6d2] shadow-sm max-w-4xl mx-auto relative">
          ...
          <Link
            href="/antar-pegawai/tahukah-anda"
          ...
          <div className="relative max-w-3xl w-full bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-700">
            <div className="flex items-center justify-between p-4 bg-slate-900 text-white">
              <span className="text-sm font-bold flex items-center gap-2">
                ...
                Pratinjau Dokumen Lampiran Tahukah Anda
              </span>
              <button
                type="button"
            ...
            <div className="p-4 flex items-center justify-center bg-slate-100 max-h-[75vh]">
              <img
                src={previewImage}
                alt="Lampiran Dokumen Tahukah Anda"
            ...
            <div className="p-4 bg-white border-t border-slate-200 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => alert('Mengunduh berkas lampiran...')}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-[#c5c6d2] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdate} className="space-y-4 mt-4">
              <div>
                <label className="block text-xs font-bold text-[#1a1b20] mb-1">Judul Artikel Tahukah Anda</label>
                <input
                  type="text"
                  required
                  value={editTitle}
              ...
              <div>
                <label className="block text-xs font-bold text-[#1a1b20] mb-1">Uraian Artikel Tahukah Anda</label>
                <textarea
                  rows={8}
                  required
                  value={editContent}
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-[#e2e3ea]">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-6 py-2 text-sm font-bold bg-[#00113a] text-white hover:bg-[#2a4386] rounded shadow-sm disabled:opacity-50 cursor-pointer"
                >
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-[#c5c6d2]">
            <div className="flex items-center gap-3 text-red-600 mb-3">
              ...
              <h2 className="text-lg font-bold text-slate-900">Konfirmasi Hapus Artikel Tahukah Anda</h2>
            </div>
            <p className="text-sm text-[#444650] leading-relaxed mb-6">
              Apakah Anda yakin ingin menghapus artikel Tahukah Anda{' '}
