...
export default function DetailTipsGayaHidupPage() {
    ...
    fetch(`/api/employee-posts/${rawId}`)
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          const matchedMock = STITCH_MOCK_TIPS_GAYA_HIDUP_9.find(
            (m) => m.id === rawId || m.title.toLowerCase().includes(rawId.toLowerCase())
          );
          ...
          if (matchedMock) {
            ...
            const fallback = STITCH_MOCK_TIPS_GAYA_HIDUP_9[0];
            setItem(fallback);
      ...
      .catch(() => {
        const matchedMock = STITCH_MOCK_TIPS_GAYA_HIDUP_9.find((m) => m.id === rawId);
        if (matchedMock) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedbackMsg('Tips dan Gaya Hidup berhasil diperbarui!');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push('/antar-pegawai/tips-gaya-hidup');
    } catch {
      router.push('/antar-pegawai/tips-gaya-hidup');
    } finally {
      setIsDeleting(false);
  ...
  const displayTitle =
    item?.title || 'Minat membaca Warga Indonesia semakin membaik';
    ...
    <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8 py-8 md:py-12 bg-white min-h-[calc(100vh-80px)] flex flex-col justify-between">
      <div>
        ...
        <div className="bg-white rounded-lg p-6 sm:p-10 border border-[#c5c6d2] shadow-sm max-w-4xl mx-auto relative">
          ...
          <Link
            href="/antar-pegawai/tips-gaya-hidup"
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-[#c5c6d2] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdate} className="space-y-4 mt-4">
              <div>
                <label className="block text-xs font-bold text-[#1a1b20] mb-1">Judul Artikel</label>
                <input
                  type="text"
                  required
                  value={editTitle}
              ...
              <div>
                <label className="block text-xs font-bold text-[#1a1b20] mb-1">Uraian Tips dan Gaya Hidup</label>
                <textarea
                  rows={8}
                  required
                  value={editContent}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-[#c5c6d2]">
            <div className="flex items-center gap-3 text-red-600 mb-3">
              ...
              <h2 className="text-lg font-bold text-slate-900">Konfirmasi Hapus Tips dan Gaya Hidup</h2>
            </div>
            <p className="text-sm text-[#444650] leading-relaxed mb-6">
              Apakah Anda yakin ingin menghapus artikel Tips dan Gaya Hidup{' '}
