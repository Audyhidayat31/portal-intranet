...
export default function EditBeritaPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/news/${id}`)
      ...
      .catch(() => {
        // Fallback mock data
        const fallbackTitle = id.startsWith('stitch-')
          ? 'Minat membaca Warga Indonesia semakin membaik'
          : 'Judul Berita';
        setJudul(fallbackTitle);
        const fallbackBody = `Jakarta – Menteri Pendidikan, Kebudayaan, Riset, dan Teknologi secara resmi membuka gedung baru Perpustakaan Nasional Republik Indonesia yang berlokasi di Jalan Medan Merdeka Selatan.\n\nGedung baru ini dirancang dengan konsep modern dan ramah lingkungan, dilengkapi dengan fasilitas teknologi informasi mutakhir untuk mendukung layanan perpustakaan digital.`;
        setDeskripsiHtml(fallbackBody);
        if (editorRef.current) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judul.trim()) {
      alert('Nama judul Berita wajib diisi');
      return;
    }
    ...
    setSuccessMessage('Artikel Berita berhasil diperbarui!');
    setTimeout(() => {
      router.push(`/kabar-kedinasan/berita/${id}`);
    }, 1200);
      ...
      <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
            ...
            {/* 5. Gambar Berita */}
            <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
              Gambar Berita <span className="float-right hidden md:inline">:</span>
            </label>
          ...
          <div className="flex gap-4 pt-6 mt-8 border-t border-[#c5c6d2]">
            ...
            <Link
              href={`/kabar-kedinasan/berita/${id}`}
