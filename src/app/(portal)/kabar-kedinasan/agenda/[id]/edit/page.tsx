...
import {
  ...
  saveStoredAgendas,
  AgendaItem,
  STITCH_MOCK_AGENDAS_6,
  MOCK_AGENDA_ATTACHMENTS_5,
} from '@/lib/mock-agendas';
...
export default function EditAgendaPage() {
  ...
  useEffect(() => {
    ...
    const currentStored = getStoredAgendas();
    const foundStored = currentStored.find((m) => m.id === id);

    if (foundStored) {
      setJudul(foundStored.title || '');
      if (foundStored.eventStartDate || foundStored.publishedAt) {
        const d = new Date(foundStored.eventStartDate || foundStored.publishedAt);
        if (!isNaN(d.getTime())) {
          setTanggal(d);
          setViewDate(d);
        }
    ...
    fetch(`/api/agendas/${id}`)
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          setJudul(item.title || '');
          if (item.eventStartDate || item.publishedAt) {
            const d = new Date(item.eventStartDate || item.publishedAt);
            if (!isNaN(d.getTime())) {
              setTanggal(d);
              setViewDate(d);
            }
          ...
          const matchedMock =
            STITCH_MOCK_AGENDAS_6.find((m) => m.id === id || m.title.toLowerCase().includes(id.toLowerCase())) ||
            STITCH_MOCK_AGENDAS_6[0];
          setJudul(matchedMock.title || '');
          if (matchedMock.eventStartDate || matchedMock.publishedAt) {
            const d = new Date(matchedMock.eventStartDate || matchedMock.publishedAt);
            if (!isNaN(d.getTime())) {
              setTanggal(d);
              setViewDate(d);
            }
      ...
      .catch(() => {
        const matchedMock =
          STITCH_MOCK_AGENDAS_6.find((m) => m.id === id) || STITCH_MOCK_AGENDAS_6[0];
        setJudul(matchedMock.title || '');
        if (matchedMock.eventStartDate || matchedMock.publishedAt) {
          const d = new Date(matchedMock.eventStartDate || matchedMock.publishedAt);
          if (!isNaN(d.getTime())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judul.trim()) {
      alert('Silakan isi judul Agenda Kegiatan');
      return;
    }
    ...
    const updatedItem: AgendaItem = {
      ...
      eventLocation: existingItem?.eventLocation || 'Gedung Perpusnas Medan Merdeka Selatan',
      status: resolvedStatus,
      authorName: existingItem?.authorName || 'Biro Hukum & Humas Perpusnas',
      attachments: existingItem?.attachments || MOCK_AGENDA_ATTACHMENTS_5,
    };

    if (existingItem) {
      saveStoredAgendas(currentStored.map((it) => (it.id === id ? updatedItem : it)));
    } else {
      saveStoredAgendas([updatedItem, ...currentStored]);
    }
    ...
    setSuccessMessage('Agenda Kegiatan berhasil diperbarui!');
    setTimeout(() => {
      router.push(`/kabar-kedinasan/agenda/${id}`);
    }, 1200);
      ...
      <div className="bg-white border border-[#c5c6d2] rounded-lg p-6 md:p-8 shadow-xs">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-[200px_auto] gap-6 mb-8 items-start">
            ...
            <div>
              <input
                ...
                placeholder="Nama judul Agenda Kegiatan"
            ...
            {/* 5. Gambar Agenda Kegiatan */}
            <label className="font-semibold text-sm sm:text-base text-[#1a1b20] pt-2">
              Gambar Agenda Kegiatan <span className="float-right hidden md:inline">:</span>
            </label>
          ...
          <div className="flex gap-4 pt-6 mt-8 border-t border-[#c5c6d2]">
            ...
            <Link
              href={`/kabar-kedinasan/agenda/${id}`}
